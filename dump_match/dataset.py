...
class Dataset(object):
    ...
    def collect(self):
        data_type = ['xs', 'ys', 'Rs', 'ts', 'ratios', 'mutuals', \
                     'cx1s', 'cy1s', 'cx2s', 'cy2s', 'f1s', 'f2s', 'pairimage_path']
        pair_idx = 0
            ...
            for seq in self.seqs:
                ...
                for i in range(seq_len):
                    for tp in data_type:
                        ...
                        if type(data_seq[tp][i]) == str:
                            imgs_path = [data_seq[tp][i]]
                            string_dt = h5py.special_dtype(vlen=str)
                            data[tp].create_dataset(str(pair_idx), data=imgs_path, dtype=string_dt)
                        else:
    ...
    def dump_data(self):
        ...
        for seq in self.seqs:
            pair_name = None if self.pair_path is None else self.pair_path + '/' + seq.rstrip("/") + '-te-' + str(
                self.pair_num) + '-pairs.pkl'
