...
class CorrespondencesDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        imgs_path = str(self.data['pairimage_path'][str(index)][:])[3:-2]
        img1_path = imgs_path.split('|')[0]
        img2_path = imgs_path.split('|')[1]

        img1 = image.imread(img1_path)
        img1 = self.process_image(image=img1, aim_H=self.image_H, aim_W=self.image_W)
        img1 = np.transpose(img1, axes=(2, 0, 1))
        img1 = np.expand_dims(img1, axis=0)
        img2 = image.imread(img2_path)
        img2 = self.process_image(image=img2, aim_H=self.image_H, aim_W=self.image_W)
